...
public class PreconditionsValidator {
    ...
    public static void verifierPreconditions(int revNetDecl1, int revNetDecl2, SituationFamiliale sitFam,
        ...
        if (nbEnfants > NB_MAX_ENFANTS) {
            throw new IllegalArgumentException("Le nombre d'enfants ne peut pas être supérieur à " + NB_MAX_ENFANTS);
        }
        ...
        boolean estSeul = sitFam == SituationFamiliale.CELIBATAIRE || sitFam == SituationFamiliale.DIVORCE || sitFam == SituationFamiliale.VEUF;
        if (estSeul && revNetDecl2 > 0) {
            throw new IllegalArgumentException("Un célibataire, un divorcé ou un veuf ne peut pas avoir de revenu déclarant 2");
        }
    }
}
